...
export async function POST(req: NextRequest) {
  try {
    ...
    if (patientError) {
      console.error("return send-code patient lookup error:", patientError);
      return NextResponse.json(
    ...
    if (caseError) {
      console.error("return send-code case lookup error:", caseError);
      return NextResponse.json(
        { error: "Unable to look up your intake right now." },
        { status: 500 }
      );
    ...
    if (isTestMode) {
      console.log("RETURN SMS TEST MODE: skipping SMS send for", normalizedPhone);
    } else {
    ...
    return NextResponse.json({ ok: true, testMode: isTestMode });
  } catch (error) {
    console.error("return send-code unexpected error:", error);
    return NextResponse.json(
      { error: "Unable to send a code right now." },
      { status: 500 }
    );
  }
}
